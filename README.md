...
14. [Đẩy code lên GitLab](#đẩy-code-lên-gitlab)
15. [Bước học tiếp theo](#bước-học-tiếp-theo)
...
├── Dockerfile
├── docker-compose.yml
├── .gitlab-ci.yml                   # CI/CD GitLab (push → build + test)
├── .github/workflows/ci.yml         # CI GitHub (tùy chọn)
...
Repository GitLab: **https://gitlab.com/codehub2026/newbie-coder-blog-api**

File **`.gitlab-ci.yml`** ở thư mục gốc — GitLab **tự chạy pipeline** mỗi khi bạn:

- **Push** code lên bất kỳ nhánh nào
- Tạo **Merge Request**
...
| Stage | Việc làm |
|-------|----------|
| `build` | `dotnet tool restore` → `restore` → `build` (Release) |
| `test` | `dotnet test` (5 tests, không cần SQL trên runner) |

### Xem kết quả trên GitLab

1. Vào project → menu **Build** → **Pipelines**
2. Chọn pipeline vừa chạy → xem job `build`, `test`
3. Biểu tượng **xanh** = thành công, **đỏ** = thất bại (bấm vào job để đọc log)

### Bật Shared Runners (lần đầu)

Nếu pipeline **pending** mãi:

1. **Settings** → **CI/CD** → mở rộng **Runners**
2. Bật **Enable shared runners for this project** (runner `gitlab.com` shared)

### GitHub Actions (tùy chọn)

Nếu mirror sang GitHub, file `.github/workflows/ci.yml` cũng build/test tương tự.

---

## Đẩy code lên GitLab

### Lần đầu (chưa có git local)

Trong thư mục dự án:

```powershell
cd c:\Users\Win\source\repos\newbie-coder-api

git init
git remote add origin https://gitlab.com/codehub2026/newbie-coder-blog-api.git

git add .
git commit -m "Initial commit: Clean Architecture API + GitLab CI"
git branch -M main
git push -u origin main
```

> Nếu đã chạy `git remote add origin` trước đó mà báo lỗi *not a git repository*, chạy `git init` trước rồi add remote lại.

### Đăng nhập GitLab khi push

- **HTTPS:** dùng [Personal Access Token](https://gitlab.com/-/user_settings/personal_access_tokens) (scope `write_repository`) thay mật khẩu
- **SSH:** `git remote set-url origin git@gitlab.com:codehub2026/newbie-coder-blog-api.git`
...
Mỗi lần `git push` → pipeline CI chạy lại tự động.
