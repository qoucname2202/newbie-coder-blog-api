...
public sealed class AuthWebApplicationFactory : TestWebApplicationFactory
{
    ...
    internal void ClearLoginState() { _authService.Reset(); TestAuthService.ClearRevokedTokens(); }
}
...
public sealed class TestAuthService : IAuthService
{
    ...
    // Static so revocation is consistent across all IAuthService instances resolved from
    // different scoped service providers (e.g. middleware vs controller).
    private static readonly HashSet<string> _revokedTokens = new();

    public static void ClearRevokedTokens() => _revokedTokens.Clear();
